...
Notion 데이터베이스 업로드를 쓰지 않을 거라면 `NOTION_DATABASE_ID`는 비워도 됩니다. 이 경우 workflow 입력의 `upload_to_notion_db`를 `false`로 둡니다.
...
upload_to_notion_db: Notion 데이터베이스 업로드 여부
```
...
Notion 데이터베이스 업로드를 켜려면:

```text
upload_to_notion_db = true
```
...
기사별 데이터베이스를 쓰려면 Notion에 새 데이터베이스를 만들고 아래 속성을 추가합니다.
...
데이터베이스 오른쪽 위 `...` 메뉴에서 integration을 연결한 뒤, 데이터베이스 URL의 ID를 GitHub Secret `NOTION_DATABASE_ID`에 저장합니다.
